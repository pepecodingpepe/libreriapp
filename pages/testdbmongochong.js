import axios from 'axios';
    ...
    const getData = async () => {
        const res = await axios.get('/api/basededatos');
    ...
    const sendData = async () => {
        setLoading(true);
        if (nombre === '' || apellido === '' || correo === '' || matricula === '' || edad === '') {
        toast.error('Llena todos los campos');
        setLoading(false);
        return;
        }
        try {
        await axios.post('/api/basededatos', {
            nombre: nombre,
            apellido: apellido,
            correo: correo,
            matricula: matricula,
    ...
    const updateData = async () => {
        ...
        try {
        await axios.put(`/api/basededatos?id=${id}`, {
            nombre: nombre,
            apellido: apellido,
            correo: correo,
            matricula: matricula,
    ...
    const eliminarData = async (id) => {
        try {
        const resultado = await axios.delete(`/api/basededatos?id=${id}`);
    ...
    return (
        ...
        <main className='p-4 bg-gray-200'>
            {data.length>0 && data.map((alumno, i) => (
